...
class VendorController extends Controller
{
    ...
    {
        parent::__construct();
        $this->repository = new BaseCRUDRepository(new Vendor());
    }
        ...
        if($request->ajax()){
            ...
            return $this->table($this->repository->query())
                ...
                ->addColumn("actions",function($row) {
                    $deleteRoute = route('admin.vendors.destroy', $row["id"]);
                    $editRoute = route('admin.vendors.update', $row["id"]);
    ...
    function destroy($vendor) {
        if($this->repository->delete($vendor)){
            $this->deletedAlert();
            return back();
        }
    }
